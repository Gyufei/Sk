...
export function IconBtn({
  ...
  return (
    <div
      ...
      onClick={handleClick}
      className={`group ${disabled ? 'is-disabled' : 'isnot-disabled'} w-12 ml-0 flex h-12 cursor-pointer items-center justify-center rounded-lg border border-[rgba(255,255,255,0.6)] data-[disabled=true]:cursor-not-allowed data-[disabled=true]:opacity-50 ${mobileHoverColorChanged ? 'data-[disabled=false]:hover:bg-[#fff]' : 'md:data-[disabled=false]:hover:bg-[#fff]'} md:ml-4 md:mt-0 md:w-12 ${className}`}
    >
      <Image
